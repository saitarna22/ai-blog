...
| POST | `/api/admin/posts/[postId]/upload-image` | 画像の手動アップロード |
...
- [x] **管理画面の改善 + シードボタン**（2026/2/9）
  - `components/admin/SeedButton.tsx` 新規作成（ペルソナ初期データ投入ボタン）
  - `app/admin/personas/page.tsx` 修正（シードボタン統合）
  - `docs/DEPLOY_GUIDE.md` 大幅拡充
- [x] **サイトタイトル修正**（2026/2/9）
  - 全公開ページ・コンポーネントのサイトタイトルを統一修正（11ファイル）
- [x] **画像生成のエラー解消**（2026/2/9）
  - `lib/generation/generatePost.ts`、`types/index.ts` の画像関連バグ修正
- [x] **画像アップロード機能 + 画像プロンプト表示**（2026/2/9）
  - `app/api/admin/posts/[postId]/upload-image/route.ts` 新規作成（管理者認証付き画像手動アップロードAPI）
  - `app/admin/posts/[postId]/page.tsx` 修正（アップロードボタン追加、DALL-Eプロンプト折りたたみ表示・コピー機能）

---

### 完了済み（未完了タスクから昇格）

- [x] **Firestoreへのペルソナ初期データ投入**（2026/2/9）
  - `SeedButton.tsx` で管理画面からワンクリック投入可能に
- [x] **画像生成失敗時のフォールバック対応**（2026/2/9）
  - 手動アップロード機能で代替画像を差し込み可能に

### 未完了タスク（今後の拡張）

#### 優先度：最高（デプロイ前に必須）
- [ ] **Vercelデプロイ実施** — 🕐 準備が整い次第すぐ
  - Vercel プロジェクト作成・環境変数設定・初回デプロイ
  - 動作確認（SSR / API Routes / Cron が正常に動くこと）

#### 優先度：高（デプロイ直後〜1週間以内）
- [ ] **構造化データ（JSON-LD）** - BlogPosting スキーマの埋め込み（SEO残タスク）。検索エンジンにインデックスされる前に入れておくと効果的
- [ ] **RSS/Atom フィード** - `/feed` エンドポイントでフィード配信。読者獲得チャネルとして公開初期に欲しい
- [ ] **SNS共有機能** - Twitter/X、LINE等のシェアボタン設置。記事が増えてきたら効果が出る
- [ ] **エラーハンドリング強化** - OpenAI API エラー時のユーザー向けメッセージ改善。運用中にエラーパターンが見えてきたら対応

#### 優先度：中（運用開始後1ヶ月目安）
- [ ] **アナリティクス導入** - Google Analytics等。公開してアクセスが来始めてから導入すれば十分
- [ ] **テスト基盤** - vitest 等の導入。機能追加やリファクタが増えてきたタイミングで

#### 優先度：低（将来検討 — ユーザーが定着してから）
...
*最終更新: 2026年2月9日（画像アップロード機能 + 画像プロンプト表示）*
